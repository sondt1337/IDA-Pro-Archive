...
static void declare_parameter_types(ea_t ea, int count)
{
  ...
  if ( sptr == nullptr )
  {
    ...
    if ( sptr == nullptr )
      return;
    add_struc_member(sptr, "methodParameterDef", -1, word_flag(), nullptr, 2);
    add_struc_member(sptr, "handlerTypeDef",     -1, byte_flag(), nullptr, 1);
  }
...
static void declare_class(ea_t ea, const char *entryname)
{
  ...
  if ( sptr == nullptr )
  {
    ...
    add_struc_member(sptr, "masterOffset", BADADDR, word_flag(), nullptr, 2);
    add_struc_member(sptr, "methodCount",  BADADDR, dec_flag()|word_flag(), nullptr, 2);
    add_struc_member(sptr, "instanceSize", BADADDR, dec_flag()|word_flag(), nullptr, 2);
    add_struc_member(sptr, "vdRelocTable", BADADDR, word_flag(), nullptr, 2);
    add_struc_member(sptr, "relocTable",   BADADDR, word_flag(), nullptr, 2);
    mt.ec.tid = get_class_struct_flags_enum();
    mt.ec.serial = 0;
    add_struc_member(sptr, "flags",        BADADDR, enum_flag()|byte_flag(), &mt, 1);
    add_struc_member(sptr, "masterMethods",BADADDR, byte_flag(), nullptr, 1);
  }
