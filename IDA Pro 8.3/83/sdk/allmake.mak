...
ifdef __ARM__
  ...
  ifeq ($(and $(__ANDROID__),$(__EA64__)),1)
...
ifneq (,$(filter $(COMPILER_NAME),gcc clang))
  ...
  CCL_REAL =        $(CROSS_PREFIX)$(_CXX)$(HOST_EXE) $(ARCH_FLAGS) $(GOLD)
  CCL = $(CCL_REAL)
...
ifneq (,$(filter $(COMPILER_NAME),gcc clang))
  ...
  ifeq ($(COMPILER_NAME),clang) # mac/android
    ...
    CC_DEFS += _GLIBCXX_USE_CXX11_ABI=0
...
ifdef IDAHOME
  ...
  ifdef NOTEAMS
    ...
    CXXFLAGS += -DENABLE_MERGE
  endif
  ifdef USE_STATIC_RUNTIME
    ifdef NOTEAMS
      # We want them to be separate for packaging in the SDK.
      EXTRASUF1:=_pro_s
    else
      EXTRASUF1:=_s
    endif
